...
/**
 * 该类是重写类Struts2中关于<s:a>标签的设定，使得<s:a>标签具有判断指定权限是否有无的功能。
 */
public class AnchorTag extends AbstractClosingTag {
   ...
   public int doEndTag() throws javax.servlet.jsp.JspException {
		   ...
		   //当前Url
		   String privilegeUrl = action; //通过<s:a action>中传递的请求来直接获得url
		   int pos = privilegeUrl.indexOf("?");
		   if(pos > -1){
			   privilegeUrl = privilegeUrl.substring(0,pos); 
		   }
		   
		   if(user.hasPrivilegeByUrl(privilegeUrl)){ //有权限，就正常输出<a>标签的内容
			   return super.doEndTag();
		   }
		   else{  //如果没有权限就不显示当前的<a>标签
			   return EVAL_PAGE; 
		   }
